...
User Route
GET Method
Schema : phone(Unique) //Required Data
Optional Data : none
Private Route
*/
  ...
  if (phone) {
    ...
    const token =
      typeof clientData.headers.token === 'string'
        ...
        : false;
    // Verify is a given token is valid for given user
    handlers._tokens.verify(token, phone, (isTokenValid) => {
      if (isTokenValid) {
        // Look up if the user exists & read
        _data.read('users', phone, (err, data) => {
          if (!err && data) {
            // Remove the hashed password & tosAgreement from the data object
        ...
        callback(401, { Error: 'Unauthorized. No Access Token is in header' });
      }
    });
  } else {
    callback(400, { Error: 'Missing Fields/Validation Error' });
...
handlers._users.put = (clientData, callback) => {
  ...
  const firstName =
    typeof clientData.payload.firstName === 'string' &&
    clientData.psayload.firstName.trim().length > 0
      ? clientData.payload.firstName.trim()
      : false;
  const lastName =
    typeof clientData.payload.lastName === 'string' &&
    ...
    _data.read('users', phone, (err, userData) => {
      if (!err && userData) {
        ...
        if (hashedPassword === userData.hashedPassword) {
          ...
          callback(401, { Error: 'Invalid Password. Unauthorized.' });
        }
      } else {
        callback(401, { Error: 'Invalid Phone Number. Unauthorized.' });
      }
    });
  } else {
...
handlers._tokens.get = (clientData, callback) => {
  ...
  if (id) {
    // Look up fot the token
    _data.read('tokens', id, (err, tokenData) => {
      if (!err && tokenData) {
        callback(200, tokenData);
      } else {
        callback(401, { Error: 'Invalid Token. Unauthorized' });
      }
    });
  } else {
    callback(400, { Error: 'Missing fields/Validation Failed' });
...
handlers._tokens.verify = (id, phone, callback) => {
  ...
  _data.read('tokens', id, (err, tokenData) => {
    if (!err && tokenData) {
      // Proceed further to match with the user
