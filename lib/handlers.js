...
handlers._checks.post = (clientData, callback) => {
  const protocol =
    typeof clientData.payload.protocol === 'string' &&
    ['http', 'https'].indexOf(clientData.payload.protocol) > -1
      ? clientData.payload.protocol
      : false;
  const url =
    typeof clientData.payload.url === 'string' &&
    clientData.payload.url.trim().length > 0
      ? clientData.payload.url.trim()
      : false;
  const method =
    typeof clientData.payload.method === 'string' &&
    ['post', 'get', 'put', 'delete'].indexOf(clientData.payload.method) > -1
      ? clientData.payload.method
      : false;
  const successCodes =
    typeof clientData.payload.successCodes === 'object' &&
    clientData.payload.successCodes instanceof Array &&
    clientData.payload.successCodes.length > 0
      ? clientData.payload.successCodes
      : false;
  const timeoutSeconds =
    typeof clientData.payload.timeoutSeconds === 'number' &&
    clientData.payload.timeoutSeconds % 1 === 0 &&
    clientData.payload.timeoutSeconds >= 1 &&
    clientData.payload.timeoutSeconds <= 5
      ? clientData.payload.timeoutSeconds
      : false;
        ...
        _data.read('users', userPhone, (err, userData) => {
          if (!err && userData) {
            ...
            if (userChecks.length < configs.maxChecks) {
              ...
              callback(400, {
                Error: `The User already exhausted max checks. Limit is ${configs.maxChecks}`,
              });
            }
          } else {
            callback(403, {
              Error: 'UnAuthorised. No User Available with this token',
            });
          }
        });
      } else {
...
handlers._checks.put = (clientData, callback) => {
  //Validate the id from clientData
  const id =
    typeof clientData.payload.id === 'string' &&
    clientData.payload.id.trim().length === 20
      ? clientData.payload.id.trim()
      : false;
  //Optional Fields
  const protocol =
    typeof clientData.payload.protocol === 'string' &&
                      ...
                      _data.update(
                        ...
                        (err) => {
                          if (!err) {
                            callback(200, {
                              Success:
                                'Deleted Check Id and Updated The User Schema',
                            });
                          } else {
                            callback(500, { error: 'server error' });
                          }
